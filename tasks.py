...
TEST_VERSION_TEMPLATE = """from dash_material_components import __version__
...
DASH_material_DIR = HERE / "dash_material_components"
...
def prerelease(ctx, version):
    ...
    run(
        ...
        "dash_material_components/_version.py "
...
def set_py_version(version):
    ...
    version_path = DASH_material_DIR / "_version.py"
...
def set_documentation_version(ctx, version):
    ...
    for iline, line in enumerate(docs_requirements):
        if "dash_material_components" in line:
            updated_line = f"dash_material_components=={version}\n"
...
def copy_dist():
    bundle_name = "dash_material_components.min.js"
    shutil.copy(
        HERE / "dash_material_components" / "_components" / bundle_name,
...
def clean(_):
    ...
    make_and_clean_dir("dash_material_components/_components")
...
def move_generated_files(_):
    ...
    dir_ = HERE / "dash_material_components"
...
@task
def build_py(ctx):
    run("dash-generate-components ./src/components dash_material_components")
    copy_dist()
...
def build_r(ctx):
    run(
        "dash-generate-components ./src/components dash_material_components "
        "--r-prefix 'dbc'"
    )
    copy_dist()
    move_generated_files(ctx)
...
def build_jl(ctx):
    ...
    with (HERE / "src" / "DashmaterialComponents.jl").open() as f:
        lines = f.readlines()
    ...
    with (HERE / "src" / "DashmaterialComponents.jl").open("w") as f:
        f.writelines(lines)
...
def install_built_packages(ctx):
    ...
    if shutil.which("julia") is not None:
        ...
        run(
            "git add -f deps/_components/dash_material_components.min.js "
